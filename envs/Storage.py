...
class Storage:
    def __init__(self, scaled_capacity, scaled_p_max, eff, min_SOC, timestep, action_size):
    ...
    @staticmethod
    def penalties_check(SOC, next_SOC, load, pv):
        penalty = 0.
    ...
    def perform_action(self, state, action, resolution):
        ...
        if balance < 0:
            current_gain = (price - avg_price) * balance
        else:
